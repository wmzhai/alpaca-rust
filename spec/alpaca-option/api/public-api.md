...
| `OptionStrategy` | class / struct | serializable strategy-owned state for positions, quantity, current underlying price, Greeks, cost, value, PnL, cashflow, spread, risk metrics, break-even fields, optional realtime peak fields, expirations, DTE, theta, score, rank, and URL; `underlying_price` is the only current-price field |
